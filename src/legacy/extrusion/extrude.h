...
#include "../../geometry.h"
#include "extrude_plan_builder.h"
...
struct extrude
{
	CGAL_TYPES(DoubleKernel)
	CGAL_EXACT_TYPES2()
	CGAL_EXACT_TYPES3()
	CGAL_ARRANGEMENT(DoubleKernel, edge2_data, face2_data, vertex2_data)
	CGAL_MESH(GeometryKernel, vertex3_data, edge3_data, face3_data)
	CGAL_UTILS(DoubleKernel, arrangement2, polyhedron3)
	...
	struct input_point
	{
		input_point(point3 p_, profile_ref profile_, uniqueid cap_label_, uniqueid vertex_id_, uniqueid edge_id_) :
			p(p_),
			profile(profile_),
			cap_label(cap_label_),
	...
	struct extruder;
	//typedef CGAL::Polyhedron_incremental_builder_3<typename geometry::HDS> incremental_builder;
	typedef std::function<void(incremental_builder& builder, geometry::polyhedron3& mesh)> extrudefn;

	static void run(input& input, geometry::polyhedron3& mesh)
	{
		class h_modifier : public CGAL::Modifier_base<geometry::HDS>
		{
			extrudefn _extruder;
			geometry::polyhedron3& _mesh;
		public:
			h_modifier(extrudefn extruder, geometry::polyhedron3& mesh) :
				_extruder(extruder),
				_mesh(mesh)
			{
			}

			void operator()(geometry::HDS& hds)
			{
				incremental_builder B(hds, true);
				B.begin_surface(0, 0);
					_extruder(B, _mesh);
				B.end_surface();
			}
		...
		auto runfn = [&input](incremental_builder& builder, geometry::polyhedron3& mesh)
		{
			extruder e(input.plane, builder, mesh, input.left_label, input.right_label, input.bottom_label, input.top_label, input.skirt_label, input.cap_label);
	...
	struct extruder
	{
		...
		typedef face_projector<geometry::Kernel, geometry::arrangement2, geometry::polyhedron3> exact_projector;
		...
		extruder(plane3 base_, incremental_builder& builder_, geometry::polyhedron3& mesh_, int default_left_, int default_right_, int default_bottom_, int default_top_, int default_skirt_, int default_cap_):
			builder(builder_),
			mesh(mesh_),
			base(base_),
			last_plane(base_),
		...
		{
			std::cout << "id: " << c->id << " [" << c->point << "], vertex: " << c->vertex << std::endl;
		}
		...
		void print_plan(plan_container& plan, bool force = false)
		{
			...
			for (auto& p : plan.plans)
			{
				...
				do
				{
					std::cout << "id: " << pit->id << " [" << pit->point << "], vertex: " << pit->vertex << std::endl;
					pit = pit->next;
				} while (pit != pnd);
		...
		void apply_edge_ids(input& input)
		{
			for (auto p : input.polygons)
			{
				for (int i = 0; i < p.size(); i++)
				{
					...
					if (curr.edge_id >= 0 && prev.vertex_index >= 0 && curr.vertex_index >= 0)
					{
						auto src = builder.vertex(prev.vertex_index);
						auto dst = builder.vertex(curr.vertex_index);
						dst->data.id = curr.vertex_id;

						auto eit = dst->vertex_begin();
						auto end = eit;
						CGAL_For_all(eit, end)
						{
							if (eit->prev()->vertex() == src)
							{
								eit->data.id = curr.edge_id;
								break;
							}
						}
			...
			for (auto& pol : i.polygons)
			{
				...
				for (int i = 0; i < pol.size(); i++)
				{
					...
					if (profile)
					{
						...
						{
							//td: error
							std::cout << "multiple signs for profile" << std::endl;
							profile = profile_ref();
						}
					}
		...
		void advance(plan_container& plan, plan_container& next_plan)
		{
			...
			if (DEBUG_COUNTER == 17)
			{
				std::cout << "DEBUG_COUNTER REACHED: " << DEBUG_COUNTER << std::endl;
			}
			_debug = true; // DEBUG_COUNTER >= 77 && DEBUG_COUNTER < 84;
#endif // DEBUG
			//print_plan(plan);
		...
		bool create_new_plan(plan_container& plan, plane3 next_plane, plan_container& next_plan, collision_list& collisions)
		{
			for (auto& c : plan.plans)
			{
				...
				do
				{
					...
					{
						std::cout << "FAR OUT VERTICES, ABORT" << std::endl;
						std::cout << pit->point << std::endl;
						std::cout << nc->point << std::endl;
						return false;
					}
		...
		profile_ref create_skirt_profile(corner_ref c, point3& profile_point, int& profile_index, int& skirt_label)
		{
			...
			typedef CGAL::Simple_cartesian<double> ProfileKernel;

			profile<ProfileKernel>::segment_list segments;
			auto found = false;
			auto p_index = 0;
			for (int i = 0; i < p->size(); i++)
			{
				...
				auto sl = p->get_skirt_label(i);
				if (sl >= 0)
					found = true;
				else
					sl = default_skirt;

				segments.push_back(profile<ProfileKernel>::segment(
					ProfileKernel::Vector_2(0, delta), sl, default_left, default_bottom, default_right, default_top, sl, false));
			...
			return profile_ref(new profile<ProfileKernel>(segments));
		}
		...
		bool is_colinear(corner_ref c, plane3* plane)
		{
			...
			auto angle = geom_utils::angle_between(vec3(c->point, c->prev->point), vec3(c->point, c->next->point));
			if (CGAL::abs(M_PI - angle) > 1e-2)
				return false;
		...
		exact_line3 corner_line(corner_ref c)
		{
			...
			if (!iline)
			{
				throw solver_error(EXTRUSION_ERROR_EDGE_CASE, "an unhandled edge case has been found.");
			}
		...
		int create_vertex(point3 p)
		{
			...
			CGAL::Cartesian_converter<DoubleKernel, ExactKernel> ek;
			auto v = builder.add_vertex(ek(p));

			//td: ids?
			//v->data.id = id;
			v->data.index = vertex_count;
		...
		void create_collision_vertex(corner_ref source, corner_ref target, point3& p)
		{
			...
			{
				if (target->vertex >= 0 && target->vertex != source->vertex)
					std::cout << "duplicate collision vertex!" << std::endl;
			...
			{
				if (source->vertex >= 0 && target->vertex != source->vertex)
					std::cout << "duplicate collision vertex!" << std::endl;
		...
		void add_face(corner_ref c, corner_ref nc)
		{
			...
			//assign labels
			f->data.label = c->labels.label;
			auto eit = f->facet_begin();
			auto end = eit;
			CGAL_For_all(eit, end)
			{
				auto v = eit->vertex();
				auto vidx = eit->vertex()->data.index;
				if (vidx == vsouth)
					eit->data.label = c->labels.bottom_label;
				else if (vidx == veast)
					eit->data.label = c->labels.right_label;
				else if (vidx == vwest)
					eit->data.label = c->labels.left_label;
				else
					eit->data.label = c->labels.top_label;
			}
		}
		...
		void close_plan(corner_ref c)
		{
			...
			CGAL_For_all(eit, end)
			{
				auto it = labels.find(eit->vertex()->data.index);
				if (it != labels.end())
					eit->data.label = it->second;
			}
		}
		...
		bool is_thin(corner_ref c)
		{
			...
			if (cc == c)
			{
				...
				std::cout << "checking thin: " << t.squared_area() << std::endl;
			...
			else if (cc->next == c)
			{
				...
				std::cout << "checking thin: " << a + t2.squared_area() << std::endl;
		...
		void build_next_plan(plan_container& plan, plane3& plane)
		{
			...
			extrude_plan_builder builder(_debug);
			extrude_plan_builder::plan_container results;
		...
		bool advance_profile(profile_ref profile, int index, point3 prev_point, point3 point, advance_profile_result& result)
		{
			...
			geometry_profile::FT dx_, dy_;
			profile->direction(index, dx_, dy_);
			profile->get_labels(index, 
				&result.labels.label, 
				&result.labels.bottom_label, 
				&result.labels.top_label, 
				&result.labels.left_label, 
				&result.labels.right_label);
			result.labels.skirt_label = profile->get_skirt_label(index);
			default_labels(result.labels);

			double dx = CGAL::to_double(dx_); 
			double dy = CGAL::to_double(dy_);
